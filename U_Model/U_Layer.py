...
class GCL_rf(GCL_basic):
    ...
    def __init__(self, nf=64, edge_attr_nf=0, reg=0, act_fn=nn.LeakyReLU(0.2), clamp=False):
        ...
        torch.nn.init.xavier_uniform_(layer.weight, gain=args.gain)
...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(), recurrent=False,
        ...
        if self.attention:
            ...
            for l_ in self.att_mlp:
                if isinstance(l_, nn.Linear):
                    xavier_uniform_(l_.weight.data, gain=args.gain2)
                    zeros_(l_.bias.data)
        ...
        for l_ in self.coord_mlp:
            if isinstance(l_, nn.Linear) and l_.bias is not None:
                xavier_uniform_(l_.weight.data, gain=args.gain)
                zeros_(l_.bias.data)
    ...
    def edge_model(self, source, target, radial, edge_attr):
        if edge_attr is None:  # Unused.
            ...
            # print(source.shape, target.shape, radial.shape, edge_attr.shape)
...
class E_GCL_mask(E_GCL):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_attr_dim=0, act_fn=nn.ReLU(), recurrent=True,
                 coords_weight=1.0, attention=False, coord_renew=False):
        E_GCL.__init__(self, input_nf, output_nf, hidden_nf, edges_in_d=edges_in_d, nodes_att_dim=nodes_attr_dim,
                       act_fn=act_fn, recurrent=recurrent, coords_weight=coords_weight, attention=attention)
        self.coord_renew = coord_renew
        if not self.coord_renew:
            del self.coord_mlp
        self.act_fn = act_fn
    ...
    def forward(self, h, edge_index, coord, node_mask=None, edge_mask=None,
                edge_attr=None, node_attr=None, n_nodes=None):
        edge_index = [edge_index[0].squeeze(), edge_index[1].squeeze()]
        row, col = edge_index
        radial, coord_diff = self.coord2radial(edge_index, coord)
